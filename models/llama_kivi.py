...
def _chunked_module_forward(module: nn.Module, hidden_states: torch.Tensor, chunk_size: Optional[int]) -> torch.Tensor:
    if chunk_size is None or chunk_size <= 0 or hidden_states.shape[1] <= chunk_size:
        return module(hidden_states)

    if torch.is_grad_enabled():
        return torch.cat(
            [module(hidden_states[:, start : start + chunk_size, :]) for start in range(0, hidden_states.shape[1], chunk_size)],
            dim=1,
        )
    ...
    for start in range(0, hidden_states.shape[1], chunk_size):
        ...
        output[:, start:end, :] = module(hidden_states[:, start:end, :])
    return output


def _chunked_norm_mlp_forward(
    norm: nn.Module, mlp: nn.Module, hidden_states: torch.Tensor, chunk_size: Optional[int]
) -> torch.Tensor:
    if chunk_size is None or chunk_size <= 0 or hidden_states.shape[1] <= chunk_size:
    ...
    if torch.is_grad_enabled():
        return torch.cat(
            [mlp(norm(hidden_states[:, start : start + chunk_size, :])) for start in range(0, hidden_states.shape[1], chunk_size)],
            dim=1,
        )
    ...
    for start in range(0, hidden_states.shape[1], chunk_size):
        ...
        output[:, start:end, :] = mlp(norm(hidden_states[:, start:end, :]))
    return output


def _chunked_value_quantize_and_pack_along_last_dim(
    data: torch.Tensor, group_size: int, bit: int, chunk_size: Optional[int]
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    if chunk_size is None or chunk_size <= 0 or data.shape[2] <= chunk_size:
        return triton_quantize_and_pack_along_last_dim(data.contiguous(), group_size, bit)

    bsz, num_heads, num_tokens, head_dim = data.shape
    feat_per_int = 32 // bit
    code = torch.empty(bsz, num_heads, num_tokens, head_dim // feat_per_int, device=data.device, dtype=torch.int32)
    scale = torch.empty(bsz, num_heads, num_tokens, head_dim // group_size, device=data.device, dtype=data.dtype)
    mn = torch.empty_like(scale)
    for start in range(0, num_tokens, chunk_size):
        end = start + chunk_size
        code_chunk, scale_chunk, mn_chunk = triton_quantize_and_pack_along_last_dim(
            data[:, :, start:end, :].contiguous(), group_size, bit
        )
        code[:, :, start:end, :] = code_chunk
        scale[:, :, start:end, :] = scale_chunk
        mn[:, :, start:end, :] = mn_chunk
        del code_chunk, scale_chunk, mn_chunk
    return code, scale, mn


def _chunked_key_quantize_and_pack_along_last_dim(
    data: torch.Tensor, group_size: int, bit: int, chunk_size: Optional[int]
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    if chunk_size is None or chunk_size <= 0:
        return triton_quantize_and_pack_along_last_dim(data.contiguous(), group_size, bit)

    chunk_size = max(group_size, (chunk_size // group_size) * group_size)
    if data.shape[-1] <= chunk_size:
        return triton_quantize_and_pack_along_last_dim(data.contiguous(), group_size, bit)

    bsz, num_heads, head_dim, num_tokens = data.shape
    feat_per_int = 32 // bit
    code = torch.empty(bsz, num_heads, head_dim, num_tokens // feat_per_int, device=data.device, dtype=torch.int32)
    scale = torch.empty(bsz, num_heads, head_dim, num_tokens // group_size, device=data.device, dtype=data.dtype)
    mn = torch.empty_like(scale)
    for start in range(0, num_tokens, chunk_size):
        end = start + chunk_size
        code_chunk, scale_chunk, mn_chunk = triton_quantize_and_pack_along_last_dim(
        ...
        code_start = start // feat_per_int
        code_end = end // feat_per_int
        group_start = start // group_size
        group_end = end // group_size
        code[..., code_start:code_end] = code_chunk
        scale[..., group_start:group_end] = scale_chunk
        mn[..., group_start:group_end] = mn_chunk
        del code_chunk, scale_chunk, mn_chunk
    return code, scale, mn
...
def _apply_rotary_pos_emb_inplace(
    ...
    for start in range(0, query_states.shape[2], chunk_size):
        end = start + chunk_size
        cos_chunk = cos[:, :, start:end, :]
        sin_chunk = sin[:, :, start:end, :]
        for states in (query_states[:, :, start:end, :], key_states[:, :, start:end, :]):
            first_half = states[..., :half].clone()
            second_half = states[..., half:].clone()
            states[..., :half] = first_half * cos_chunk[..., :half] - second_half * sin_chunk[..., :half]
            states[..., half:] = second_half * cos_chunk[..., half:] + first_half * sin_chunk[..., half:]
...
class LlamaAttention_KIVI(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.value_quant_chunk_size = getattr(config, "value_quant_chunk_size", 8192)
        self.rotary_chunk_size = getattr(config, "rotary_chunk_size", 8192)
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_states, key_states = _apply_rotary_pos_emb_inplace(
            query_states, key_states, cos, sin, position_ids, self.rotary_chunk_size
        )
        ...
        if past_key_value is not None:
            ...
            if value_states.shape[-2] <= self.residual_length:
                ...
                value_states_quant = value_states[:, :, :-self.residual_length, :]
                value_states_full = value_states[:, :, -self.residual_length:, :].contiguous()
                value_states_quant, value_scale, value_mn = _chunked_value_quantize_and_pack_along_last_dim(
                    value_states_quant, self.group_size, self.v_bits, self.value_quant_chunk_size
                )
...
class LlamaDecoderLayer_KIVI(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = _chunked_module_forward(self.input_layernorm, hidden_states, self.mlp_chunk_size)
        ...
        # Fully Connected
        residual = hidden_states
        hidden_states = _chunked_norm_mlp_forward(
            self.post_attention_layernorm, self.mlp, hidden_states, self.mlp_chunk_size
        )
        hidden_states = residual + hidden_states
...
class LlamaModel_KIVI(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        hidden_states = _chunked_module_forward(self.norm, hidden_states, self.norm_chunk_size)
...
class LlamaForCausalLM_KIVI(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        logits_device = hidden_states.device
        if labels is None:
            # Generation only needs the last position's logits.
            # Avoids materializing full-sequence float32 logits (OOM on long prompts).
        ...
        logits = logits.float()
        if logits.device != logits_device:
